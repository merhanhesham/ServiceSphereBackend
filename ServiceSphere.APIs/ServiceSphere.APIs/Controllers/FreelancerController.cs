...
{ 
    public class FreelancerController : BaseController
    {
        ...
        [HttpGet("Freelancers")]
        public async Task<IActionResult> GetFreelancers([FromBody] FreelancerSpecParams? freelancerParams)
        {
            // Initialize query
            IQueryable<Freelancer> query = _serviceSphereContext.Freelancers.Include(f => f.Categories).Include(f => f.SubCategories);
